#!/bin/bash
# Yi Zheng 2023/2024 Trabajo de Fin de Grado
# Este script es una ligera modificación del código de Javier Ortiz de Artiñano.
# El objetivo de este script es generar una base de datos de algunos de los géneros de la familia Rhizobiaceae
# Géneros usados: Rhizobium, Bradyrhizobium, Mesorhizobium, Sinorhizobium, Neorhizobium, Georhizobium, Pararizhobium y Pseudorhizobium.
# Dataset (found in data/):
#	- index.txt -> index of RefSeq assemblies to be analyzed. Every row contains the following fields separated by tabs:
...
#array=(Rhizobium Bradyrhizobium Mesorhizobium Sinorhizobium Neorhizobium Georhizobium Pararizhobium Pseudorhizobium)
...
ayuda () {
   ...
   echo -e '\nusage: DBGenerator.bash -o <output_folder> -f <file_organisms> [-i] [-a] [-A] [-h]\n'
   echo -e '-o output_folder: proyect folder in which data and results will be stored.'
   echo -e "-f file_organisms: text file with the name of the organisms that will be used to create the DB.\n\t\tEach line must be a name of the organism."
...
indexGenerator () {
	echo -e "\tGenerating master index of assemblies..."
	...
	while IFS= read -r organism
	do
		esearch -db assembly -query "($organism[Organism] OR $organism[All Fields]) AND ('latest refseq'[filter] AND all[filter] NOT anomalous[filter])" < /dev/null | 
...
while getopts ho:f:ibaA option; do
	case $option in
		h) ayuda; exit 0;;
		o) projectName=$OPTARG;;
		f) txtOrganism=$OPTARG;;
		i) updateIndex=true;;
...
#To make sure that at least the output folder name and list of organisms have been provided.
if [ "$#" -lt 4 -o "$projectName" = false -o "$txtOrganism" = false ]; then
